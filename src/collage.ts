...
async function toCellBuffer(base64: string): Promise<Buffer | null> {
  try {
...
// rasmga birlashtiradi, shunda Claude'ga har bir model uchun faqat bitta
// rasm yuboriladi, lekin u bir nechta burchakni ko'rsatadi. Natija diskka
// (Railway volume) keshlanadi; rasmlar to'plami o'zgarmaguncha qayta
// yasalmaydi.
export async function getModelCollage(
  ...
  try {
    ...
    for (const dl of downloads) {
      if (!dl) continue;
      const cell = await toCellBuffer(dl.base64);
      if (cell) cellBuffers.push(cell);
    }
    if (cellBuffers.length === 0) return null;
